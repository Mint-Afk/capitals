...
The module is gonna be called directly to manage database or through the main app 
EGC-2K20 to check if the user is registered and allow him access to functionalities.
...
def new_user(user, passw):
    ...
    if results:
        print("Sorry, username [{}] is already in use. Please choose another one!"
               .format(user))
    else:
...
if __name__ == "__main__":
    ...
    if args.add and args.rm or args.add and args.check or args.rm and args.check:
        print("ATTENTION!! You cannot execute both operations at the same time!") 
        quit()
    if args.add:
        ...
        if check_user(args.usr, args.psw):
            print("Credentials you have inserted for user [{}] are the correct ones!"
                  .format(args.usr))
        else:
            print("Sorry, wrong or invalid password selected!")
    else:
        print("Please select the operation you wanna execute [-add], [-rm] or [-check]!")
        conn.close()
